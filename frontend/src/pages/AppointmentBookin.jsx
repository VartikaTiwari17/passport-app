    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Select your preferred Passport Seva Kendra and time slot</p>
        </div>
            ...
            {LOCATIONS.map((loc) => (
              <label key={loc.name}
                className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition
                  ${!loc.slots ? 'opacity-40 cursor-not-allowed bg-gray-50' :
                    selectedLocation === loc.name ? 'border-blue-500 bg-blue-50' :
                    'border-gray-200 hover:border-blue-300'}`}>
                <input type="radio" name="location" value={loc.name}
                  disabled={!loc.slots}
                ...
                {!loc.slots && <span className="text-xs text-red-400 whitespace-nowrap">No slots</span>}
              </label>
            ))}
          </div>
        </div>
            ...
            {dates.map((date) => (
              <button key={date} onClick={() => setSelectedDate(date)}
                className={`p-2 rounded-lg text-xs text-center border transition
                  ${selectedDate === date ? 'bg-blue-900 text-white border-blue-900' :
              ...
              {TIME_SLOTS.map((time) => (
                <button key={time} onClick={() => setSelectedTime(time)}
                  className={`p-2 rounded-lg text-xs border transition
                    ${selectedTime === time ? 'bg-blue-900 text-white border-blue-900' :
                      'border-gray-200 hover:border-blue-400 text-gray-700'}`}>
                  {time}
                </button>
              ))}
        ...
        <div className="flex gap-3">
          <button onClick={() => navigate(`/apply/${appId}/documents`)}
          ...
          <button onClick={handleBook}
            disabled={booking || !selectedLocation || !selectedDate || !selectedTime}
            className="flex-1 bg-green-600 text-white py-2.5 rounded-lg text-sm font-medium hover:bg-green-700 transition disabled:opacity-40 disabled:cursor-not-allowed">
